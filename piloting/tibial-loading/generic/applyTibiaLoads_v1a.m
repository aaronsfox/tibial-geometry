...
%%%% Adapted version of original approach, applying the joint forces to a
%%%% singular node between the malleoli
%%%%
%%%% This also results in serious bending --- but reverts back to the
%%%% localised nodal force when done in a dynamic fashion (static results
%%%% in ultra deformation...)
...
febio_spec.MeshData.NodeData{1}.node.VAL = forceTotal(1);
...
febio_spec.MeshData.NodeData{2}.node.VAL = forceTotal(2);
febio_spec.MeshData.NodeData{2}.node.ATTR.lid = (1:1:numel(jointPointID))';
%Force Z
...
febio_spec.MeshData.NodeData{3}.node.VAL = forceTotal(3);
febio_spec.MeshData.NodeData{3}.node.ATTR.lid = (1:1:numel(jointPointID))';
